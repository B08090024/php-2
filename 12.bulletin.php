<?php
    error_reporting(0);
    session_start();
    if (!isset($_SESSION["id"])){ //SESSION["id"]
        echo "請登入系統";
        echo "<meta http-equiv='refresh' content='3; url=login.html'>";//以login.html
    }else{
        ...
        $result=mysqli_query($conn, "select * from bulletin");//$conn :qurry數據庫服務氣的連接 "select * from bulletin"
